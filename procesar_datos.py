...
modelo.add(Dropout(0.2)) # Apaga el 20% de las neuronas al azar para evitar que el modelo se memorice los datos (overfitting)

# Capa de Salida: Una neurona por cada seña posible, usando softmax para dar un porcentaje de probabilidad
modelo.add(Dense(NUM_CLASES, activation='softmax'))

# 4. Compilar el modelo
modelo.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['categorical_accuracy'])

# 5. Configurar "Callbacks" (Herramientas durante el entrenamiento)
# Guarda automáticamente la mejor versión del modelo
